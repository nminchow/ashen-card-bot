...
module.exports = (id, { releases, name, open }) => {
  ...
  const setupInstructions = open ? '' : ' React with the following to exclude (or re-include) releases from the draft:';

  const description = `Your draft has been created!${setupInstructions}`;
  ...
  const completionInfo = open ? '' : 'Once ready, issue the ✅ reaction to lock joining, shuffle, and deal each participant their first draft hand.';
